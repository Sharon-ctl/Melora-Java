...
public class EmbedHelper {
    ...
    public static Container createLyricsContainer(String lyricsId, String query, List<String> pages, int pageNum,
            String source, boolean isLive) {
        StringBuilder content = new StringBuilder();
    ...
    public static Container createLiveLyricsContainer(String trackTitle, String trackAuthor,
            List<com.discord.musicbot.lyrics.KaraokeManager.LrcLine> lines, int activeIndex, String statusMessage) {
        StringBuilder content = new StringBuilder();
        content.append("### ").append(com.discord.musicbot.config.EmojiConfig.getInstance().music).append(" [LIVE SYNC] Lyrics: ")
                .append(escapeMarkdown(trackTitle != null ? trackTitle : "Unknown Track")).append("\n");
        if (trackAuthor != null && !trackAuthor.isBlank() && !trackAuthor.equalsIgnoreCase("Spotify")) {
            content.append("**").append(escapeMarkdown(trackAuthor)).append("**\n\n");
        } else {
            content.append("\n");
        }
        ...
        if (statusMessage != null && !statusMessage.isBlank()) {
            ...
            List<com.discord.musicbot.lyrics.KaraokeManager.LrcLine> window = com.discord.musicbot.lyrics.KaraokeManager
                    .getActiveLineWindow(lines, activeIndex, 2, 5);
            for (com.discord.musicbot.lyrics.KaraokeManager.LrcLine line : window) {
                long totalSeconds = line.timestampMs / 1000;
            ...
            content.append("No synced lyrics available for this track.\n");
        }

        content.append("\n-# Live tracking • Updates instantly as playback progresses");
    ...
    public static Container createHelpContainer(String category, String prefix, JDA jda) {
        ...
        if (category.equals("home")) {
            ...
            description.append("**").append(com.discord.musicbot.config.EmojiConfig.getInstance().mewsic)
                    .append(" Official Mewsic Collaboration**\n");
            description.append(
                    "This bot is integrated with the offline Mewsic app. You can play your Mewsic playlists seamlessly using `/mewsic play` and `/mewsic import`.\n\n");
            ...
            description
                    .append(String.format("**%splay <song/url>** - Play a song or add it to the queue.\n\n", prefix));
            description.append(String.format(
                    "**%splayinstant <song/url>** - Play a song immediately, skipping the current track.\n\n", prefix));
            description.append(
                    String.format("**%splaynext <song/url>** - Add a song to the top of the queue.\n\n", prefix));
            description.append(String
                    .format("**%splayrandom [source]** - Play a random track from favorites or history.\n\n", prefix));
            description.append(
                    String.format("**%ssearch <query>** - Search for a song and pick from a list.\n\n", prefix));
            ...
            description.append(
                    String.format("**%sprevious** - Play the previously played track from history.\n\n", prefix));
            description.append(
                    String.format("**%sreplay** - Replay the currently playing track from the beginning.\n\n", prefix));
            ...
            description
                    .append(String.format("**%sdisconnect** - Disconnect the bot from the voice channel.\n\n", prefix));
            ...
            description.append(String.format(
                    "**%sfilter <filter>** - Apply audio filters (bassboost, 8d, vaporwave, etc).\n\n", prefix));
            description.append(
                    String.format("**%scrossfade [duration]** - Enable PCM crossfading between tracks.\n\n", prefix));
            ...
            description.append(String
                    .format("**%squeue show [page] [user]** - View all tracks in the current queue.\n\n", prefix));
            description.append(String
                    .format("**%squeue search <query>** - Search for a specific track within the queue.\n\n", prefix));
            description.append(
                    String.format("**%squeue deduplicate** - Scan and remove any duplicate entries.\n\n", prefix));
            description.append(
                    String.format("**%squeue compact** - Group identical tracks together sequentially.\n\n", prefix));
            description.append(String
                    .format("**%squeue reverse** - Invert the current play order of all queued tracks.\n\n", prefix));
            description.append(
                    String.format("**%squeue sort <criteria>** - Organize queue by title or author.\n\n", prefix));
            description.append(String
                    .format("**%squeue slice <start> <end>** - Remove a contiguous range of tracks.\n\n", prefix));
            description.append(String
                    .format("**%squeue shufflefrom <position>** - Shuffle tracks after a specific point.\n\n", prefix));
            description.append(String
                    .format("**%squeue swap <pos1> <pos2>** - Exchange the positions of two tracks.\n\n", prefix));
            description.append(String.format(
                    "**%squeue export** - Export the current queue into a downloadable JSON file.\n\n", prefix));
            description.append(String.format(
                    "**%squeue import <file>** - Import a JSON queue file to append to the active queue.\n\n", prefix));
            description.append(
                    String.format("**%snowplaying** - View details about the currently playing track.\n\n", prefix));
            description
                    .append(String.format("**%sshuffle** - Randomize the order of tracks in the queue.\n\n", prefix));
            description.append(String.format("**%sloop <mode>** - Set repeat mode: off, track, or queue.\n\n", prefix));
            description.append(
                    String.format("**%sremove <number>** - Remove a specific track from the queue.\n\n", prefix));
            description.append(String
                    .format("**%sinsert <song/url> <position>** - Insert a song at a specific position.\n\n", prefix));
            description.append(
                    String.format("**%smove <from> <to>** - Move a track to a different position.\n\n", prefix));
            description.append(String.format("**%sclear** - Remove all tracks from the queue.\n\n", prefix));
            description.append(
                    String.format("**%sjump <number>** - Skip directly to a specific track in the queue.\n\n", prefix));
            ...
            description.append(
                    String.format("**%splaylist add <playlist> [query]** - Add tracks to a playlist.\n\n", prefix));
            ...
            description.append(
                    String.format("**%smewsic import <file>** - Import a Mewsic playlist JSON file.\n\n", prefix));
            description.append(String.format(
                    "**%smewsic export <playlist>** - Export a Melora playlist to Mewsic JSON format.\n", prefix));
            ...
            description.append(
                    String.format("**%ssettings** - (Admin) Open the server configuration dashboard.\n\n", prefix));
            description.append(
                    String.format("**%sseek <time>** - Jump to a specific timestamp in the track.\n\n", prefix));
            description
                    .append(String.format("**%stime** - View the current playback time and progress bar.\n\n", prefix));
            description.append(String.format(
                    "**%slyrics [query]** - Get lyrics for the currently playing track or a search query.\n\n",
                    prefix));
            ...
            description.append(
                    String.format("**%sdjmode** - (Admin/DJ) Configure DJ Mode and temporary access.\n\n", prefix));
            description.append(String.format("**%sblacklist** - (Admin) Manage guild music blacklist.\n\n", prefix));
            description.append(String
                    .format("**%sgrab** - Send the currently playing track to your Direct Messages.\n\n", prefix));
        ...
        if (category.equals("home")) {
            ...
            return Container.of(
                    section,
                    createHelpMenu()).withAccentColor(COLOR_MAIN);
        }
    ...
    public static Container createCommandHelpContainer(String commandName, String prefix, JDA jda) {
        ...
        switch (commandName) {
            case "play":
                ...
                break;
            case "playinstant":
                description = "Play a song immediately, skipping the current track.\n\n**Usage:** `" + prefix
                        + "playinstant <song/url>`";
                break;
            case "playnext":
                description = "Add a track to the top of the queue (plays next).\n\n**Usage:** `" + prefix
                        + "playnext <query>`";
                break;
            case "playrandom":
                description = "Play a random track from your favorites or the bot's global history.\n\n**Usage:** `"
                        + prefix + "playrandom [source]`";
                break;
            case "search":
                description = "Searches for a song and gives you a menu to select from.\n\n**Usage:** `" + prefix
                        + "search <query>`";
                break;
            case "pause":
            ...
            case "skip":
                ...
                break;
            case "previous":
                description = "Play the previously played track from history.\n\n**Usage:** `" + prefix + "previous`";
                break;
            case "replay":
                description = "Replay the currently playing track from the beginning.\n\n**Usage:** `" + prefix
                        + "replay`";
                break;
            case "forward":
                description = "Fast-forwards by a specific amount of seconds.\n\n**Usage:** `" + prefix
                        + "forward <seconds>`";
                break;
            case "rewind":
                description = "Rewinds by a specific amount of seconds.\n\n**Usage:** `" + prefix + "rewind <seconds>`";
                break;
            case "stop":
            ...
            case "disconnect":
                ...
                break;
            case "autoplay":
                description = "Toggle automatic recommendations after the queue finishes.\n\n**Usage:** `" + prefix
                        + "autoplay`";
                break;
            case "crossfade":
                description = "Enable and configure PCM crossfading between tracks.\n\n**Usage:** `" + prefix
                        + "crossfade [duration]`";
                break;
            case "247":
                description = "Keep the bot in the voice channel 24/7 even if no music is playing. Can optionally lock the bot to restrict dismissal.\n\n**Usage:** `"
                        + prefix + "247 [lock]`";
                break;
            case "queue":
            case "queue show":
                description = "View all tracks in the current queue. You can optionally filter to only see tracks queued by a specific user.\n\n**Usage:** `"
                        + prefix + "queue show [page] [user]`";
                break;
            case "queue search":
                description = "Search for a specific track currently within the queue.\n\n**Usage:** `" + prefix
                        + "queue search <query>`";
                break;
            case "queue deduplicate":
                description = "Scan the queue and remove any duplicate entries automatically.\n\n**Usage:** `" + prefix
                        + "queue deduplicate`";
                break;
            case "queue compact":
                description = "Group identical tracks together in the queue sequentially.\n\n**Usage:** `" + prefix
                        + "queue compact`";
                break;
            case "queue reverse":
                description = "Invert the current play order of all queued tracks.\n\n**Usage:** `" + prefix
                        + "queue reverse`";
                break;
            case "queue sort":
                description = "Organize the queue alphabetically by track title or track author.\n\n**Usage:** `"
                        + prefix + "queue sort <criteria>`";
                break;
            case "queue slice":
                description = "Remove a contiguous range of tracks from the queue.\n\n**Usage:** `" + prefix
                        + "queue slice <start> <end>`";
                break;
            case "queue shufflefrom":
                description = "Shuffle only the tracks that occur after a specific point in the queue.\n\n**Usage:** `"
                        + prefix + "queue shufflefrom <position>`";
                break;
            case "queue swap":
                description = "Exchange the positions of two specific tracks in the queue.\n\n**Usage:** `" + prefix
                        + "queue swap <pos1> <pos2>`";
                break;
            case "queue export":
                description = "Export the current queue into a downloadable JSON file.\n\n**Usage:** `" + prefix
                        + "queue export`";
                break;
            case "queue import":
                description = "Import an existing JSON queue file to append to the active queue.\n\n**Usage:** `"
                        + prefix + "queue import <file>`";
                break;
            case "nowplaying":
                description = "View details about the currently playing track.\n\n**Usage:** `" + prefix
                        + "nowplaying`";
                break;
            case "shuffle":
                ...
                break;
            case "loop":
                description = "Set repeat mode: off, track, or queue.\n\n**Usage:** `" + prefix + "loop <mode>`";
                break;
            case "remove":
                description = "Remove a specific track from the queue by its number.\n\n**Usage:** `" + prefix
                        + "remove <number>`";
                break;
            case "removedupes":
                description = "Remove all duplicate tracks from the queue.\n\n**Usage:** `" + prefix + "removedupes`";
                break;
            case "insert":
                description = "Insert a song at a specific position in the queue.\n\n**Usage:** `" + prefix
                        + "insert <song/url> <position>`";
                break;
            case "move":
                description = "Move a track from one position to another.\n\n**Usage:** `" + prefix
                        + "move <from> <to>`";
                break;
            case "clear":
                description = "Remove all tracks from the queue but keep playing the current track. You can optionally filter to only clear tracks queued by a specific user.\n\n**Usage:** `"
                        + prefix + "clear [user]`";
                break;
            case "jump":
                description = "Skip directly to a specific track in the queue.\n\n**Usage:** `" + prefix
                        + "jump <number>`";
                break;
            case "playlist":
                description = "Manage custom playlists.\n\n**Usage:** `" + prefix + "playlist <subcommand>`";
                break;
            case "playlist add":
                description = "Add a track to a playlist. If the playlist doesn't exist, it will be created automatically.\n\n**Usage:** `"
                        + prefix + "playlist add <playlist> [query]`";
                break;
            case "playlist create":
                description = "Create a new playlist.\n\n**Usage:** `" + prefix + "playlist create <name>`";
                break;
            case "playlist delete":
                description = "Delete a playlist.\n\n**Usage:** `" + prefix + "playlist delete <name>`";
                break;
            case "playlist rename":
                description = "Rename a playlist.\n\n**Usage:** `" + prefix + "playlist rename <old> <new>`";
                break;
            case "playlist list":
                description = "View your playlists.\n\n**Usage:** `" + prefix + "playlist list [page]`";
                break;
            case "playlist info":
                description = "View playlist info.\n\n**Usage:** `" + prefix + "playlist info <name>`";
                break;
            case "playlist tracks":
                description = "View tracks in a playlist.\n\n**Usage:** `" + prefix + "playlist tracks <name> [page]`";
                break;
            case "playlist addqueue":
                description = "Add current queue to a playlist.\n\n**Usage:** `" + prefix
                        + "playlist addqueue <playlist>`";
                break;
            case "playlist remove":
                ...
                break;
            case "playlist move":
                description = "Move a track in a playlist.\n\n**Usage:** `" + prefix
                        + "playlist move <playlist> <from> <to>`";
                break;
            case "playlist dedupe":
                description = "Remove duplicate tracks from a playlist.\n\n**Usage:** `" + prefix
                        + "playlist dedupe <name>`";
                break;
            case "playlist play":
                description = "Play a playlist.\n\n**Usage:** `" + prefix + "playlist play <playlist>`";
                break;
            case "playlist instant":
                description = "Force-play a playlist immediately.\n\n**Usage:** `" + prefix
                        + "playlist instant <playlist>`";
                break;
            case "playlist export":
                description = "Export a playlist as JSON.\n\n**Usage:** `" + prefix + "playlist export <playlist>`";
                break;
            case "playlist import":
                description = "Import a playlist from JSON.\n\n**Usage:** `" + prefix + "playlist import <file>`";
                break;
            case "favorites":
                description = "Manage your favorite tracks.\n\n**Usage:** `" + prefix + "favorites <subcommand>`";
                break;
            case "favorites add":
                description = "Add a track to your favorites.\n\n**Usage:** `" + prefix + "favorites add [query]`";
                break;
            case "favorites remove":
                ...
                break;
            case "favorites list":
                description = "View your favorites.\n\n**Usage:** `" + prefix + "favorites list [page]`";
                break;
            case "favorites play":
                description = "Play your favorites.\n\n**Usage:** `" + prefix + "favorites play`";
                break;
            case "favorites instant":
                description = "Force-play your favorites immediately.\n\n**Usage:** `" + prefix + "favorites instant`";
                break;
            case "mewsic":
                description = "Integration with the offline Mewsic app.\n\n**Usage:** `" + prefix
                        + "mewsic <subcommand>`";
                break;
            case "mewsic import":
                description = "Import a Mewsic playlist JSON file.\n\n**Usage:** `" + prefix + "mewsic import <file>`";
                break;
            case "mewsic export":
                description = "Export the current queue or a saved playlist to Mewsic JSON format.\n\n**Usage:** `"
                        + prefix + "mewsic export <source> [playlist]`";
                break;
            case "volume":
                description = "Adjust the playback volume (1-200).\n\n**Usage:** `" + prefix + "volume <1-200>`";
                break;
            case "ping":
                description = "Check bot latency.\n\n**Usage:** `" + prefix + "ping`";
                break;
            case "time":
                description = "View the current playback time and progress bar.\n\n**Usage:** `" + prefix + "time`";
                break;
            case "lyrics":
                description = "Get lyrics for the currently playing track or a search query.\n\n**Usage:** `" + prefix
                        + "lyrics [query]`";
                break;
            case "stats":
                description = "View your personal listening statistics.\n\n**Usage:** `" + prefix + "stats`";
                break;
            case "history":
                description = "View or clear your listening history.\n\n**Usage:** `" + prefix
                        + "history <subcommand>`";
                break;
            case "savedqueue":
                description = "Manage your saved queues.\n\n**Usage:** `" + prefix + "savedqueue <subcommand>`";
                break;
            case "wrapped":
                description = "View your personalized " + jda.getSelfUser().getName()
                        + " Wrapped stats.\n\n**Usage:** `" + prefix + "wrapped`";
                break;
            case "karaoke":
                description = "Toggle Live Karaoke mode for the current session. Synced lyrics will appear in the Now Playing embed if available.\n\n**Usage:** `"
                        + prefix + "karaoke`";
                break;
            case "vote":
                description = "Start a vote.\n\n**Usage:** `" + prefix + "vote <skip/clear/disconnect>`";
                break;
            case "djmode":
                description = "Configure DJ Mode settings and grant temporary access.\n\n**Usage:** `" + prefix
                        + "djmode <on/off/role/grant/revoke>`\n\n- `on/off`: Enable or disable DJ mode (Requires Manage Server)\n- `role <role>`: Set the DJ role (Requires Manage Server)\n- `grant <user>`: Give a user temporary DJ access for the session\n- `revoke <user>`: Revoke temporary access";
                break;
            case "blacklist":
                description = "Manage guild music blacklist.\n\n**Usage:** `" + prefix + "blacklist <subcommand>`";
                break;
            case "grab":
                description = "Send the currently playing track to your Direct Messages.\n\n**Usage:** `" + prefix
                        + "grab`";
                break;
            case "filter":
                description = "Apply an audio filter to the playback.\n\n**Filters:** `bassboost`, `earrape`, `pop`, `rock`, `electronic`, `nightcore`, `vaporwave`, `8d`, `tremolo`, `vibrato`, `distortion`, `muffled`, `vocal_remove`, `mono`, `clear`\n\n**Usage:** `"
                        + prefix + "filter <filter>`";
                break;
            case "settings":
                description = "Open the server configuration dashboard to toggle bot behaviors (Admin only).\n\n**Usage:** `"
                        + prefix + "settings`";
                break;
            default:
                description = "Command not found. Use `" + prefix + "help` to see all commands.";
                break;
        }

        return Container.of(
                TextDisplay.of("### Command: " + commandName + "\n\n" + description)).withAccentColor(COLOR_MAIN);
    }
    ...
    public static List<ActionRow> createNowPlayingComponents(MusicManager manager) {
        ...
        row1.add(Button.secondary("np_loop",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnLoop)));
        row1.add(Button.secondary("np_previous",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnPrevious)));
        row1.add(Button.secondary("np_pause",
                Emoji.fromFormatted(paused ? com.discord.musicbot.config.EmojiConfig.getInstance().btnResume
                        : com.discord.musicbot.config.EmojiConfig.getInstance().btnPause)));
        row1.add(Button.secondary("np_skip",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnSkip)));
        row1.add(Button.secondary("np_shuffle",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnShuffle)));

        // Row 2: Queue, Volume Down, Stop, Volume Up, Favorite
        row2.add(Button.secondary("np_queue",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnQueue)));
        row2.add(Button.secondary("np_voldown",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnVolumeDown)));
        row2.add(Button.danger("np_stop",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnStop)));
        row2.add(Button.secondary("np_volup",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnVolumeUp)));
        row2.add(Button.secondary("np_fav",
                Emoji.fromFormatted(com.discord.musicbot.config.EmojiConfig.getInstance().btnFavorite)));
    ...
    public static Container createHistoryContainer(
            java.util.List<com.discord.musicbot.data.HistoryManager.HistoryEntry> history, int page) {
        ...
        String footer = "Page " + page + "/" + maxPages + " | " + totalTracks + " track"
                + (totalTracks != 1 ? "s" : "");
        content.append("\n-# ").append(footer);
    ...
    public static Container createPlaylistListContainer(List<PlaylistData> playlists, int page,
    ...
    public static Container createSettingsContainer(GuildSettings settings) {
        StringBuilder desc = new StringBuilder();
        desc.append("### ").append(com.discord.musicbot.config.EmojiConfig.getInstance().settings)
        ...
        desc.append("**Voice Channel Status:** ").append(settings.isUpdateVcStatus() ? enabledEmoji : disabledEmoji)
                .append("\n");
        desc.append("**Announce Tracks:** ").append(settings.isAnnounceTracks() ? enabledEmoji : disabledEmoji)
                .append("\n");
    ...
    public static List<ActionRow> createSettingsComponents(GuildSettings settings) {
        ...
        Button statusBtn = Button.secondary("setting_status",
                "VC Status: " + (settings.isUpdateVcStatus() ? "ON" : "OFF"));
        Button announceBtn = Button.secondary("setting_announce",
                "Announce: " + (settings.isAnnounceTracks() ? "ON" : "OFF"));
        Button mode247Btn = Button.secondary("setting_247", "24/7: " + (settings.isMode247() ? "ON" : "OFF"));
        Button autoplayBtn = Button.secondary("setting_autoplay",
                "Autoplay: " + (settings.isAutoplay() ? "ON" : "OFF"));
